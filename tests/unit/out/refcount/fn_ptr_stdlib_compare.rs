...
pub fn my_alternative_fwrite_1(p: Ptr<u8>, n: u64, m: u64, f: AnyPtr) -> u64 {
    let p: Value<Ptr<u8>> = Rc::new(RefCell::new(p));
    let n: Value<u64> = Rc::new(RefCell::new(n));
    let m: Value<u64> = Rc::new(RefCell::new(m));
    let f: Value<AnyPtr> = Rc::new(RefCell::new(f));
...
fn main_0() -> i32 {
    ...
    let gn2: Value<FnPtr<fn(Ptr<u8>, u64, u64, AnyPtr) -> u64>> = Rc::new(RefCell::new(
        FnPtr::<fn(AnyPtr, u64, u64, Ptr<::std::fs::File>) -> u64>::new(libcc2rs::fwrite_refcount)
            .cast::<fn(Ptr<u8>, u64, u64, AnyPtr) -> u64>(Some(
            (|a0: Ptr<u8>, a1: u64, a2: u64, a3: AnyPtr| -> u64 {
                libcc2rs::fwrite_refcount(
                    a0.to_any(),
    ...
    assert!({
        let _lhs = (*gn1.borrow()).clone();
        _lhs == ((*gn2.borrow()).cast::<fn(AnyPtr, u64, u64, Ptr<::std::fs::File>) -> u64>(None))
            .clone()
    });
    let g3: Value<FnPtr<fn(AnyPtr, u64, u64, Ptr<::std::fs::File>) -> u64>> =
        Rc::new(RefCell::new(
            FnPtr::<fn(Ptr<u8>, u64, u64, AnyPtr) -> u64>::new(my_alternative_fwrite_1).cast::<fn(
                AnyPtr,
                u64,
                u64,
                Ptr<::std::fs::File>,
    ...
    'loop_: loop {
        ...
        ));
        {
            ((buf.as_pointer() as Ptr<u8>) as Ptr<u8>).to_any().memset(
                ...
                ::std::mem::size_of::<[u8; 10]>() as u64 as usize,
            );
            ((buf.as_pointer() as Ptr<u8>) as Ptr<u8>).to_any().clone()
        };
