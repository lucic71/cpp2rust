...
fn main_0() -> i32 {
    ...
    assert!(
        ((*(*it.borrow()).second().borrow()) == (*sentinel.borrow()))
            && (!(Ptr::from_string_literal(
                b"iterator does not have correct value before insert\0"
            ))
            .is_null())
    );
    assert!(
        ...
        }) && (!(Ptr::from_string_literal(b"pointer does not have correct value before insert\0"))
            .is_null())
    );
    ...
    assert!(
        ((*(*it.borrow()).second().borrow()) != 0)
            && (!(Ptr::from_string_literal(
                b"in refcount, iterator points to index 0 instead of sentinel\0"
            ))
            .is_null())
    );
    assert!(
        ((*(*it.borrow()).second().borrow()) == (*sentinel.borrow()))
            && (!(Ptr::from_string_literal(
                b"iterator does not have correct value after insert\0"
            ))
            .is_null())
    );
    assert!(
        ({
            let _lhs = ((*p.borrow()).read());
            _lhs == (*sentinel.borrow())
        }) && (!(Ptr::from_string_literal(b"pointer does not have correct value after insert\0"))
            .is_null())
    );
