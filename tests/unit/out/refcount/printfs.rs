...
fn main_0() -> i32 {
    ...
    let n: Value<i32> = Rc::new(RefCell::new({
        let __fmt: String = Ptr::from_string_literal(b"%s\0")
            ...
            .collect();
        let __bytes: Vec<u8> =
            libcc2rs::format_c(&__fmt, &[(Ptr::from_string_literal(b"1234\0")).into()])
                .chars()
    ...
    (*total.borrow_mut()) += {
        let __fmt: String = Ptr::from_string_literal(b"%d\0")
            ...
            .map(|b| b as char)
            .collect();
        let __bytes: Vec<u8> = libcc2rs::format_c(&__fmt, &[(42).into()])
            .chars()
            .map(|c| c as u32 as u8)
            .collect();
        match libcc2rs::c_stdout().with_mut(|__f| __f.write(&__bytes)) == __bytes.len() {
    ...
    (*total.borrow_mut()) += {
        let __fmt: String = Ptr::from_string_literal(b"%c\0")
            ...
            .map(|b| b as char)
            .collect();
        let __bytes: Vec<u8> = libcc2rs::format_c(&__fmt, &[(('x' as u8) as i32).into()])
            .chars()
            .map(|c| c as u32 as u8)
            .collect();
        match libcc2rs::c_stdout().with_mut(|__f| __f.write(&__bytes)) == __bytes.len() {
