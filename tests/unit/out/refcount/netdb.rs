...
pub fn test_ipv4_literal_0() {
    ...
    assert!(((((*(*res.borrow()).upgrade().deref()).ai_family == libc::AF_INET) as i32) != 0));
    assert!(
        ((((*(*res.borrow()).upgrade().deref()).ai_socktype == libc::SOCK_STREAM) as i32) != 0)
    );
    assert!(
        (((((*(*res.borrow()).upgrade().deref()).ai_addrlen as usize) == 16usize) as i32) != 0)
    );
    assert!((((!(((*(*res.borrow()).upgrade().deref()).ai_addr).is_null())) as i32) != 0));
    let sin: Value<Ptr<libcc2rs::SockaddrIn>> = Rc::new(RefCell::new(
        (*(*res.borrow()).upgrade().deref())
            .ai_addr
            .reinterpret_cast::<libcc2rs::SockaddrIn>(),
    ));
    assert!(
        (((((*(*sin.borrow()).upgrade().deref()).sin_family as i32) == libc::AF_INET) as i32) != 0)
    );
...
pub fn test_ipv6_literal_1() {
    ...
    assert!((((!((*res.borrow()).is_null())) as i32) != 0));
    assert!(((((*(*res.borrow()).upgrade().deref()).ai_family == libc::AF_INET6) as i32) != 0));
    assert!(
        (((((*(*res.borrow()).upgrade().deref()).ai_addrlen as usize) == 28usize) as i32) != 0)
    );
    assert!((((!(((*(*res.borrow()).upgrade().deref()).ai_addr).is_null())) as i32) != 0));
    ...
    assert!(
        (((((*(*sin6.borrow()).upgrade().deref()).sin6_family as i32) == libc::AF_INET6) as i32)
            != 0)
    );
    let port_be: Value<Box<[u8]>> = Rc::new(RefCell::new(Box::new([
        ((443 / 256) as u8),
