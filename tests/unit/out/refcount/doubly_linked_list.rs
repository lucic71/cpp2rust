...
fn main_0() -> i32 {
    ...
    assert!(
        (({ Find_0((*head.borrow()).clone(), 0) }).with(|__v| __v
            .next
            .clone()
            .with(|__v| __v.val)
            == 3))
    ...
    assert!(
        (({ Find_0((*head.borrow()).clone(), 2) }).with(|__v| __v
            .prev
            .clone()
            .with(|__v| __v.val)
            == 3))
    ...
    assert!(
        ({
            ...
            ({ FindBack_1((*tail.borrow()).clone(), 0) }).with(|__v| _lhs + __v.val)
        } == (4 + -1_i32))
    );
    assert!({
    ...
    assert!({
        ...
        ({ FindBack_1((*tail.borrow()).clone(), 4) }).with(|__v| _lhs == (__v.prev.clone()))
    });
    return 0;
}
